...
## ADR-024: サブエージェントの結果が返らない — 全エージェント定義に `SendMessage` が無かった

### Context
- 派生プロジェクト（how-we-work）で `design-critique` から `design-critic` を起動したところ、**結果が5時間戻ってこなかった**
- 「重い処理で回り続けている」と誤認し、2体目を立てて**二重に走らせる**という二次被害も出た
- ログを調べると `type=system, subtype=turn_duration: 35297ms` ——**35秒で完走していた**。批評結果は完全に書き上がっており、**親に配送されないまま待機していただけ**だった
- グローバル設定が `teammateMode: tmux`。この形態では Agent で起動したものは「結果を親に返す Task サブエージェント」ではなく、**独立セッションのチームメイト**になる。チームメイトは最終テキストを自動返却せず、`SendMessage` で明示的に返す設計
- ⚠️ しかし **`code-reviewer` / `design-critic` / `design-drafter` の3つとも `tools:` に `SendMessage` が無かった**。＝ **返す手段が無い状態で完走する**

### Decision
- 3つのエージェント定義の `tools:` に **`SendMessage` を追加**
- ⚠️ **tools に足すだけでは不十分**と判断し、各定義の末尾に「**⚠️ 結果の返し方（必読）**」節を追加して、`SendMessage({ to: "team-lead", ... })` で返すことを**明記**した
  （足しただけだと「最終テキストで返る」という既定の思い込みで使わない可能性がある。ツールの有無と、使う判断は別物）
- `.claude/rules/tool-usage.md` に「**サブエージェントの結果が返らないとき**」の診断手順を追加。ログの `turn_duration` で完走判定できること、text ブロックから結果を回収できることを明文化
- `design-critique` スキルにも同趣旨の注記（このループが最初に踏む場所のため）

### Consequences
- (+) `/basic-review` `/deep-review`（code-reviewer）、`design-mock` 等（design-drafter）、`design-critique`（design-critic）が**結果を返せるようになった**。これらは `auto-task` / `auto-design` / `auto-build` の内部で呼ばれるため、**自律ループが同じ場所で止まる事故を防げる**
- (+) 「戻らない＝重い」と誤認せず、**ログで完走判定する**手順が残った
- (−) `tools:` を絞る意図（文脈遮断・最小権限）に `SendMessage` という例外を1つ持ち込んだ。ただし SendMessage は**結果を出す方向**であり、批評者に文脈を流し込む経路ではないので、文脈遮断は壊れない
- **ADR-023 と同じ構造の失敗**: あちらは「同じ欠陥がファイルをまたいで存在した」。今回も**3ファイル全部が同じ欠落**を持っていた。→ `tool-usage.md` に「同じ欠落は全エージェント定義にある前提で点検する」を追記した

### Alternatives
- `teammateMode` を tmux から変える → グローバル設定で影響範囲が広い。他のワークフローが tmux 前提の可能性があり、局所の不具合を直すために全体の挙動を変えるのは筋が悪い
- 呼び出し側が毎回「結果をファイルに書いて」と指示する → `design-critic` には Write が無いので結局 tools を触る必要がある。かつ呼ぶたびに指示を書く運用は忘れる
- ログから回収する運用にする → 今回の応急処置としては機能したが、毎回手作業になる。仕組みで直すべき
- `tools:` の指定自体をやめて全ツール継承にする → 文脈遮断・最小権限の設計意図を捨てることになる。必要な1つだけ足すほうが小さい
## ADR-025: 送り状取り込み（how-we-work）— 公式一次情報の判断軸を制度・手続きへ拡張する

- **Context**:
  `CLAUDE.md` には既に「公式ドキュメント/言語思想に整合。Qiita/Zennは他出典で裏が取れるときのみ採用」という規約があったが、
  これは**技術情報**を想定した書き方で、**制度・規約・料金・行政手続き**を明示的にカバーしていなかった。
  how-we-work で実際に2件の失敗が起きている（どちらも同じ構造）:
  1. 個人ブログの実録記事を一次情報として扱い「可能」と結論したが、公式は明確に否定していた。結果、費用と時間のかかる手続きへ誘導した
  2. サブスク解約で公式でない方法を前提に検討を進めた。公式には無料の正規手順があった

  共通構造は「検索要約・体験談で結論し、公式の文言を自分で読まなかった」こと。
  そして**ユーザーには「公式が何と言っているか」が見えないまま判断させた**。

- **Decision**:
  1. `.claude/skills/official-source-first/SKILL.md` を新設（判断軸スキル）。
     出典階層・「書かれていない≠否定されている」の区別・無料経路の先行確認・出力の型を規定する
  2. `CLAUDE.md` の基本原則に1行追加し、既存の技術情報向け規約を**制度・手続きへ明示的に拡張**する
  3. `.claude/rules/dev-practices.md` の禁止項目に「無料の正規経路を確認せずに費用が出る手順を提案する」を追加。
     既存の課金規定は**実行**の規定であり、**提案・調査段階**が抜けていた穴を塞ぐ
  4. 別モデルCLIでの白紙クロスチェックは、独立スキルにせず `official-source-first` の一節として内蔵する

- **Consequences**:
  - 判断軸（事後学習）とクロスチェック（事前防御）が1枚に収まり、高リスクな問いで両方が同時に効く
  - 出典元は180行あったが、テンプレート側では**104行に圧縮**した。
    `/template-feedback` の最小主義（厚いスキルは低コストモデルの誤読・過剰実行を誘発する）に従い、
    失敗の経緯とクロスチェックの実績値は要点だけに畳んだ。判断軸そのもの（鉄則6つ・出力の型・アンチパターン）は落としていない
  - クロスチェックのCLIは環境依存なので、**「使えるなら使う／無い環境では省略してよい」**の条件付き記述にした。
    特定CLIの存在をテンプレートの前提にしない

- **Alternatives**:
  - **クロスチェックを別スキルに分割**: 役割は明確になるが、ファイルが増え、片方だけ読まれるリスクがある。
    今回は「高リスクな問いでは両方セットで効く必要がある」ため内蔵を採用
  - **出典を全量移植（180行）**: 情報は落ちないが、テンプレート内で最も厚いスキルになる。
    最小主義に反するため不採用

- **出典**: how-we-work `doc/output/to-template.md` 項目1〜3（2026-08-13 のセッション）

---

## ADR-026: 一時的な設計書と確定的な設計書を分け、昇格に合意の関所を置く

- **Context**:
  「話していないことが、いつのまにか要件になっている」という違和感を、
  このマシンに残る全セッションログ（40件・695人間ターン）から追跡した（2026-08-27 のレポート）。

  実測でわかったこと:
  - 残存ログの全期間が `claude-opus-5`。**モデルの違いでは説明できない**（指標も時期差なし: 〜08-19 38% / 08-20〜 38%）
  - 合意せず着手する古典的な「先回り」はむしろ減っている（宣言なし編集は 08-20 以降 0.5%）
  - 起きていたのは **作業の過程で生まれた設計判断が、そのまま要件のSSOTへ書き込まれること**
  - 「着手前スコープ宣言」を出した24回のうち **9回（38%）は人間の応答を挟まずそのまま実行へ入っていた**
  - how-we-work の `rdd.md` に追加された断定形の設計判断6件は、**追跡できた全件がAI初出**

  構造は「人間の曖昧な感想 → AIが断定的な規範へ変換 → 宣言 → GOを待たず実行 →
  実行中に生まれた判断がSSOTへ → 次セッションでは決定事項」。
  各段に明白な違反は無く、**手続き上は合意されている**ため、確認や宣言を足しても防げない。

  さらに `proto-loop` は工程として `rdd.md` への反映経路を持ち（SKILL.md 63行目）、
  同じ構造の穴が `auto-design` / `auto-task` / `design-mock` / `design-ssot` の
  「未記入なら短問で確認し、回答を rdd.md に反映」にも**横展開されていた**（計6箇所）。

- **Decision**:
  1. **置き場所を分ける**: `doc/draft/`（未合意・発酵中）を新設し、`doc/input/`（合意済み・SSOT）と並置する。
     パスを見れば「前提にしてよいか」が判別できる状態にする
  2. **昇格を候補提示に変える**: `proto-loop` ステップ5で、AIは昇格候補を提示するだけにする。
     上げるかどうかはAIが判断しない
  3. **rdd反映の前に関所を置く**: ステップ7で、要件になりうるものを一覧提示し、
     **書き込む文面そのもの**を見せて合意を得てから書く
  4. **迂回路を塞ぐ**: ラウンド中の `doc/input/` への直接書き込みを禁止する（書けるのは `doc/draft/` だけ）
  5. **横展開**: 他4スキルの6箇所にも「書き込む文面を見せて合意を得てから書く」を追加
  6. **サイトで状態を見せる**: `build-context-site` に `doc/draft/` を含め、
     確定/発酵中を**状態バッジで区別**して並べることを必須にする
  7. `apply_template.sh` は `doc/draft/README.md` だけを配布し、
     各プロジェクトの発酵物には触らない（`sync` の `--delete` で消さない保護）

- **Consequences**:
  - プロトの完走と要件への昇格が構造的に分離される。「どこで要件に追加されたか」が追える
  - 昇格のたびに人間の応答が要るため、**ループの連続実行は途中で止まる**。
    これは意図した効果（判断は人間が握るという前提の実装）
  - `doc/draft/` は `.gitignore` に入れない。発酵物もバージョン管理する
    （消えてよいが、経緯は追えるべき）
  - ⚠️ **工程に書いただけでは迂回できる**。8/18 の実例は設定集ごと迂回して直接SSOTへ書いていた。
    機械的なガード（`PreToolUse` フックで `doc/input/` への書き込みを検知）は
    性質が違うため**別PRに分離**した。工程だけでは「守られているつもり」が残る

- **Alternatives**:
  - **`doc/input/draft/` と入れ子にする**: ディレクトリ数は増えないが、
    「input の中に未合意のものがある」状態が残るため不採用
  - **`fermenting/` 等の固有名**: プロジェクトの語彙（思考の発酵場所）には合うが、
    初見での判別が `draft` より弱いため不採用
  - **フックを同時に入れる**: 迂回はすぐ塞がるが、`doc/input/` への書き込みごとに確認が入り、
    日常の確認回数が増える。効き目を見てから判断するため分離

- **出典**: 2026-08-27 のセッションログ解析（`doc/generated/reports/2026-08-27-ai-judgment-ssot.html`）

---

## ADR-027: 確定物への書き込みは「止める」のではなく「見せる」

- **Context**:
  ADR-026 で工程に関所を置いたが、**工程に書いただけでは迂回できる**。
  実際 2026-08-18 のセッションでは、設定集を経由せずプロト作成の途中で直接 `rdd.md` へ書き込んでいた。
  ガード系フックで繰り返し確認された「守られているつもりが最も危険」と同じ構造。

  同時に、レビューの実態にも変化があった。
  以前は IDE でファイルを開いて内容を確認してからコミットしていたが、現在は CLI だけで完結させている。
  CLI で全差分（今回のPRは624行）を読むのは現実的でなく、結果として
  **確定物に何が入ったかが見えないままコミットされる**。

  ただし全行を読む必要は無い。今回の事故は確定物への差分（67行）の中でしか起きていない。
  **「全部見る/見ない」ではなく「確定物だけ必ず見る」に絞れば、CLIで完結できる**。

  さらにスキル横断で調査したところ、同型のバグが2型見つかった:
  - **型A**: 見た目の調整をAIが解釈してデザインSSOTへ永続化（`design-mock` 2箇所 / `design-html` 1箇所）。
    どのトークンに落とすかは設計判断だが、合意なく確定していた
  - **型B**: `auto-build` の「rdd.md の不明点を**黙って**推測で埋める（埋めた場合は必ず記録）」が、
    **記録すれば埋めてよい**と読める形になっていた

- **Decision**:
  1. `PreToolUse` で**止める**のではなく、`PostToolUse` で**差分を見せる**フックにする。
     作業は流れたまま、見えないまま入ることだけを無くす（確認回数を増やさない）
  2. **ツール名で判定しない**。AIは `Write`/`Edit` ではなく `Bash`（sed / heredoc / python）で書き換える。
     ツール名で絞ると素通りするため、**git の作業ツリーの差分**で判定する
  3. 同じ差分を繰り返し出さない（`.git/ssot-diff-shown` にハッシュを保持）
  4. 差分が60行以下なら本文を出し、超えたら stat と確認コマンドの案内に留める（端末を埋めない）
  5. 型A・型Bを修正し、SSOTへ反映する前に**差分そのものを見せて合意**を得る形にする

- **Consequences**:
  - `meta/adr-lite.md` に AI が書いた文面も、書いた瞬間に差分として出る。
    ADR-026 で積み残していた「ADRの文面を見せる」問題が、ルールを足さずに解決する
  - 止めないため、**見た人が止めなければ通る**。これは意図した設計（判断は人間が握るが、作業は妨げない）
  - フックはプロジェクトの `.claude/settings.json` に登録した。
    グローバルへは配らない（`apply_global.sh` は hooks を上書きするため、利用者の設定を壊しうる）
  - 検証は `test-show-ssot-diff.sh`（10ケース）。**見せる/黙るの両方**を実測している。
    特に「Bash の heredoc / sed で書き換えた場合も検知する」ケースを含む

- **Alternatives**:
  - **`PreToolUse` で止める**: 迂回はすぐ塞がるが、確定物への書き込みごとに確認が入り日常の確認回数が増える。
    ADR-026 の時点で分離した判断を踏襲し、まず「見せる」で効き目を見る
  - **`matcher` を `Write|Edit` だけにする**: 既存フックと揃うが、**Bash 経由の書き換えが素通りする**（実測で確認）
  - **差分を全部出す**: 見落としは減るが端末が埋まり、結局読まれなくなる。60行で閾値を切った

- **出典**: 2026-08-27 のセッション（ADR-026 の積み残し＋スキル横断調査）

---

## ADR-028: 変更が出たら差分ビューアを1回だけ自動で開く

- **Context**:
  ADR-027 で確定物の差分を端末に出すようにしたが、対象は確定物だけで、実装差分は見えないままだった。
  一方、レビューの実態は「IDE を開かず CLI だけで完結させる」に変わっている。
  `hunk`（agent-authored changesets 向けの差分ビューア）と `herdr` は既に導入済みだが、
  **起動する操作を人間が覚えていないと使われない**。

- **Decision**:
  1. `PostToolUse` で、変更が出たら `hunk` を split ペインで**1回だけ**開く。
     編集のたびに開き直すと画面が何度も分割されるため、2回目以降は開かない
  1b. ⚠️ **実測で前提が1つ覆った**: 当初「hunk は watch 状態なので一度開けば追随する」と考えていたが、
     実際には開いたセッションは新しい変更を拾わなかった（`fileCount` が増えず `updatedAt` も進まない）。
     そのため、起動済みの場合は `hunk session reload --repo <path> -- diff` を明示的に呼ぶ。
     ペインは開き直さないので画面は分割されない
  1c. ⚠️ **実測でもう1つ覆った**: 「変更が無ければ何もしない」とすると、
     コミット直後や変更を戻した直後に、hunk が**消えた差分を表示し続ける**。
     「まだ変更が残っている」という誤解を生むため、変更が無いときも
     起動中なら reload して空の状態を反映させる
  2. **「開いたか」を状態ファイルで推測しない**。`hunk session list --json` で
     daemon に実際の起動状態を問い合わせる。人間が手で閉じた場合も正しく判定できる
  3. **投げっぱなし（`&`）にしない**。結果を確認できない形は「開いたつもり」を生む
     （サブエージェントが結果を返せず黙って詰まった ADR-024 と同じ構造）。
     応答しない場合は `timeout` が守る
  4. `herdr` の外では何もしない。`CLAUDE_AUTO_HUNK=0` で無効化できる
  5. あわせてシェル関数 `sabun` を用意し、キーバインドを覚えなくても
     コマンドで差分を見られるようにする（worktree / staged / branch / 確定物 の4モード）

- **Consequences**:
  - 画面が自動で分割される。好まない場合は環境変数で切れる
  - `hunk` が無い環境では何も起きない（フックは黙って終わる）。`sabun` は `git diff` にフォールバックする
  - 検証は `test-auto-open-hunk.sh`（8ケース）。開く/開かない両方を実測。
    `herdr` と `hunk` はスタブに差し替えるため、テスト中に実際のペインは開かない

- **テスト作成時の学び（記録）**:
  最初のテストは2件が誤検出していた。原因は**テスト自身の不備**で、フックは正しく動いていた。
  1. スタブを sandbox の**中**に置いたため、`git status` に未追跡ファイルとして出て「変更あり」になった
  2. `PATH` にシステムパスが残り、スタブを消しても**本物の `hunk`** が見つかっていた
  → ガードの検証では、**テストの分離が甘いと「動いていない」と誤判定する**。
     実装を疑う前に、テストの前提を実測する

- **Alternatives**:
  - **編集のたびに開く**: 見落としは減るが、画面が何度も分割されて実用に耐えない
  - **状態ファイルで開いたかを記録**: 実装は簡単だが、人間が手で閉じた場合に追随できない
...
## ADR-030: 現在日時をフックで毎ターン注入する

- **Context**:
  AI は自前で「今」を知らない。放置すると学習時点の日付やセッション開始時の日付で
  相対表現を解釈するため、スケジュール調整・締切・「今週/来週/明日」の会話がズレる。
  Claude Code 内でこの手の会話をする機会が増えており、実害が出る前提の問題になっていた。
  `SessionStart` だけでは**起動時の日時で固定される**。朝に立ち上げたセッションで夕方に
  「明日の予定」と言われると、AI は朝の時点で考えてしまう。

- **Decision**:
  1. **`SessionStart` ＋ `UserPromptSubmit` の2段構え**で日時を注入する。
     後者により「発言したその瞬間の日時」が毎ターン入り、長時間セッションでもズレない
  2. 注入内容は **日付＋曜日＋時刻＋タイムゾーン＋ISO週番号**。
     「今週中」「来週の頭」で**週の境界**を正しく取るため曜日と週番号を含める。
     曜日はロケール設定に依存させないよう、スクリプト内の配列で日本語表記を持つ
  3. 置き場所は **グローバル（`~/.claude/settings.json`）一本**とし、
     配布物（`.claude/settings.json`）には置かない。
     hooks は階層マージでグローバルとプロジェクトの**両方が発火する**ため（ADR-005 で確認済み）、
     両方に置くと ai-template 自身での作業時に日時が二重注入される。実際に二重発火を確認し、
     配布物側を撤去して1回になることまで実測した
  4. スクリプト実体と復元手順は `meta/personal-env/` に保全する。
     `meta/` は `apply_template.sh` の配布対象外＝他プロジェクトには流れない

- **Consequences**:
  - (+) 全プロジェクトで常に正しい「今」を前提に会話できる。スケジュール調整の事故が減る
  - (+) 二重注入が起きない。トークン消費も1ターン2行と軽微
  - (−) グローバル専用のため、ai-template を他者が使う場合は
    `meta/personal-env/README.md` の復元手順を手動で踏む必要がある
  - (−) 毎ターンの注入なので、`UserPromptSubmit` に他フックが増えると起動コストが積み上がる（現状は数ms）

- **Alternatives**:
  - **`SessionStart` のみ**: 実装は最小だが、長時間セッションで必ずズレる。
    今回の目的（スケジュール調整）を満たさないため不採用
  - **配布物側（`.claude/settings.json`）に置く**: 全プロジェクトに配れるが、
    階層マージで ai-template 内が二重発火する。公開よりも確実に効くことを優先して不採用
  - **AI に必要時だけ `date` を実行させる**: 「日時を意識すべき場面」の判定自体を AI に委ねることになり、
    意識しそこねる可能性が残る。関所として機能しないため不採用

- **出典**: 2026-08-28 のセッション（二重発火の実測＋`meta/personal-env/README.md` の置き場所表との照合）
