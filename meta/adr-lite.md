...
## ADR-008: 現在日時をフックで毎ターン注入する

- **日付**: 2026-08-28
- **対象**: `~/.claude/settings.json`（グローバル）, `meta/personal-env/hooks/inject-current-datetime.sh`

### Context
- AI は自前で「今」を知らないため、放置すると学習時点の日付やセッション開始時の日付で相対表現を解釈する
- Claude Code 内でスケジュール調整・締切の会話をする機会が多く、「今日」「来週」がズレると実害が出る
- `SessionStart` のみだと起動時の日時で固定される。長時間セッション（朝立ち上げ→夕方に会話）で必ずズレる

### Decision
- **`SessionStart` ＋ `UserPromptSubmit` の2段構え**で日時を注入する。後者により「発言したその瞬間の日時」が毎ターン入る
- 注入内容は **日付＋曜日＋時刻＋タイムゾーン＋ISO週番号**。「今週中」「来週の頭」で週の境界を正しく取るため曜日と週番号を含める
- 置き場所は **グローバル（`~/.claude/settings.json`）一本**。配布物（`.claude/settings.json`）には置かない
  - 理由: hooks は階層マージで両方発火するため（ADR-005 で確認済み）、両方に置くと ai-template 自身での作業時に日時が二重注入される
- スクリプト実体は `meta/personal-env/hooks/` に保全（`meta/` は `apply_template.sh` の配布対象外＝他プロジェクトに流れない）

### Consequences
- (+) 全プロジェクトで常に正しい「今」を前提に会話できる。スケジュール調整の事故が減る
- (+) 二重注入が起きない。トークン消費も1ターン2行と軽微
- (−) グローバル専用のため、ai-template を他者が使う場合は `meta/personal-env/README.md` の復元手順を手動で踏む必要がある
- (−) 毎ターンの注入なので、`UserPromptSubmit` に他フックが増えると起動コストが積み上がる（現状は数ms）

### Alternatives
- `SessionStart` のみ → 長時間セッションで必ずズレる。今回の目的（スケジュール調整）を満たさない → 不採用
- 配布物側（`.claude/settings.json`）に置く → 全プロジェクトに配れるが、階層マージで ai-template 内が二重発火 → 不採用
- AI に必要時だけ `date` を実行させる → 「日時を意識すべき場面」の判定自体を AI に委ねることになり、意識しそこねる可能性が残る → 不採用
