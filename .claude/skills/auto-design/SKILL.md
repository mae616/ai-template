...
- 技術スタック（UI フレームワーク）・ターゲット表示環境は `doc/input/rdd.md` に従う。**未定義・未記入なら推測せず短問で確認**し、回答を rdd.md に反映してから進む（rdd.md 自体が無い場合も同様）
...
- **`art-direction` を読み込む**: ステップ2（確認用HTML）・ステップ3（静的UI骨格）の描画前に**AD宣言**を出し、提示前に自己審査ゲート（あからさま検知）を通す。確定した宣言は `project-design-language` の固有値レジストリへ同期する
