...
```
- **`doc/input/rdd.md` を読み込む**: 技術スタック・制約・非機能要件を実装の前提にする。実装判断に必要な項目が**未記入なら推測で埋めず短問で確認**し、回答を rdd.md に反映してから進む
- **UI・画面・コンポーネントを触る Issue の場合**は `project-design-language` を参照し、トーン・固有値（色/タイポ/モーション/文言）の判断根拠にする
  ...
  - ただし **`auto-build` の後行モード連鎖中**（人間が既に「デザイン後行」を選択済み）は、その選択を「TBD 明記の仮値で進める」合意とみなし、**停止せず**後行時実装規約（`dev-practices.md`: SSOT参照の仮値・セマンティック構造）で進めてよい
