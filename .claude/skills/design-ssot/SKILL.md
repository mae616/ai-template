...
- **`doc/input/rdd.md` を読み込む**: ターゲット表示環境・技術スタック・制約をSSOT抽出の前提にする。実装判断に必要な項目が**未記入なら推測で埋めず短問で確認**し、回答を rdd.md に反映してから進む（⚠️ **書き込む文面を見せて合意を得てから書く**。会話での一言を、AIが作った文面のまま要件として定着させない）
