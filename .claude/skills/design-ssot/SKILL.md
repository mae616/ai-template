...
- **`doc/input/rdd.md` を読み込む**: ターゲット表示環境・技術スタック・制約をSSOT抽出の前提にする。実装判断に必要な項目が**未記入なら推測で埋めず短問で確認**し、回答を rdd.md に反映してから進む
- **`project-design-language` を読み込む**: 基本方針（誰のため/北極星/単一メタファー）を抽出時の解釈（トーン・命名・variants の切り方）の前提にする
  - **雛形のまま**（基本方針が `{...}` / TBD）なら、`judgment-harness` の発酵ループに従い、**基本方針だけを質問して人間と埋めてから**抽出に進む（Figma の内容から手がかりを拾って候補提示してよい。固有値レジストリは TBD のままでよく、抽出した SSOT と同期する）

## 見た目の基準（ビューポート）について
- まず `doc/input/rdd.md` の「ターゲット表示環境（事実）」を参照し、**そのビューポートを基準**にSSOTを作る
- 未記入の場合は、まず**短問で確認して回答を rdd.md に反映**する。すぐに確認できない場合のみ、以下を **推奨デフォルト**として仮置きし、出力やレビューの前提に明記する：
  - desktop: 1440x900
  - mobile: 390x844
  - tablet: 834x1194
...
### SSOT迂回（直書き）を見つけたときの昇格手順
色/文言/効果値の直書きをレビューで検出したら、次の順で昇格する:
1. SSOT（`design-tokens.json` / `copy.json` 等）へ値を追加
2. 実装側ミラー（型/定数）へ追加
3. SSOT⇔ミラーの一致テストを追加（乖離を検知するガード）
4. 直書き箇所をSSOT参照に差し替える
