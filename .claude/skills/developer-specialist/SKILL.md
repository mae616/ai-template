...
description: 設計と実装を「最小で正確」に進め、差分思考で品質と速度を両立する。設計の溶け込み（責務不明/重複/暫定対応）を防ぎ、レビュー可能な変更へ落とし込むときに使う。
---
...
5. 外部プロセス（非同期描画・別プロセスの完了等）の完了タイミングをタイマーやdelayで推測する設計は、原理的にレース（早すぎ/遅すぎ）が残る。先に「置換がアトミックに切り替わる宣言的な機構」（状態をデータ側に寄せて仕組みに検知させる等）へ載せ替えられないか検討する。載せ替えられれば制御コード自体が消え、タイミング依存もなくなる。
