...
| 成果物 | 置き場 | 性質 |
|---|---|---|
| 設定集（Setting Book） | `doc/input/design/setting-book/`（ソースMD。`index.html` に統合してアーティファクト公開） | 発酵場所。ビジュアル付き |
| プロトタイプ | `proto/*` ブランチ または `samples/proto-N/`（本番ビルド・配布から除外） | 使い捨て |
| Figmaプロト | Figmaファイル（URLを設定集に記録） | 使い捨て〜SSOT種 |
| 確定した判断 | `project-design-language` スキルへ昇格 | SSOT |
...
- `project-design-language` に昇格できる確定判断があれば昇格（迷ったら設定集に留める。`judgment-harness` の「メモとSSOTを分ける」）
- 人間と次を決める: **もう1ラウンド回すか、収束に向かうか**
...
- `project-design-language` の昇格漏れを最終確認
