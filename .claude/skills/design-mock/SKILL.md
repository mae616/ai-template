...
- **`doc/input/rdd.md` を読み込む**: ターゲット表示環境・技術スタック・制約を叩き台の前提にする。実装判断に必要な項目が**未記入なら推測で埋めず短問で確認**し、回答を rdd.md に反映してから進む（rdd.md 自体が無い場合も同様に、必要最小限の事実を質問して起票する）
...
- **`art-direction` を読み込む**: HTMLを描く直前に**AD宣言**（コンセプト導出線/材質/禁止則/シグネチャ表現）を出してから描画に入る。描画後は自己審査ゲート（あからさま検知）を通してから提示する
...
- `art-direction` の自己審査ゲートを通過している（汎用テンプレ化していない/材質の矛盾なし/シグネチャ表現を1つ指させる/カード数・塗り数が禁止則内）。結果はAD宣言と併せて提示に添える
