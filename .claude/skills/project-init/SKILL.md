---
user-invocable: true
description: "[プロジェクト] 壁打ち → 要件定義 → ボイラーテンプレート作成 → AIテンプレート適用を一気通貫"
---
...
- 技術スタック選定は公式ドキュメント・デファクトスタンダードに基づく。
...
| `doc/guide/` | `doc/guide/` | ガイドドキュメント群 |
...
# ガイドドキュメントをコピー
cp -r "$TEMPLATE_SOURCE/doc/guide/" "$PROJECT_DIR/doc/guide/"
```
