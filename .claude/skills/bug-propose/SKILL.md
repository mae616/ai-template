---
user-invocable: true
description: "3. トラブルシューティング修正策で仮説→合理的な修正策の列挙"
---
...
- 出典ポリシーは `CLAUDE.md` の原則、および `.claude/rules/ai-guidelines.md` の「情報源・引用（出典）ポリシー（詳細）」に従い、**根拠を伴う対応策** を整理する
...
- 詳細は `.claude/rules/ai-guidelines.md` を参照
...
## 検索・評価ポリシー（参照）
詳細は `.claude/rules/ai-guidelines.md` の「情報源・引用（出典）ポリシー（詳細）」に従う（= `CLAUDE.md` の原則を具体化）。
