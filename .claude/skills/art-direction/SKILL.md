...
- 描画後の批評は `design-critique`（デザイン版レビューループ）が担う。ルーブリック（10問）のSSOTも同スキル
...
- 正式な批評は `design-critique` に委ねる: **文脈遮断したサブエージェント**（スクリーンショット＋AD宣言のみを渡す）がルーブリック10問で違和感を言語化し、**must-fix 0 まで反復**（最大3周）
