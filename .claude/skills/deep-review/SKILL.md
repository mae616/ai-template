...
- **実装と同じ文脈でこのレビューを実行しない**（`basic-review` と同じ原則。実装者は自分の設計判断を「意図どおり」としか採点できない）
- 実装ループから呼ばれた場合は、**`code-reviewer` サブエージェント**（`.claude/agents/code-reviewer.md`）に `モード: deep` と PR番号/ブランチ名**だけ**を渡して実行する（文脈遮断≠情報遮断: レビュアーは差分・rdd.md・設計書・SSOTを自分で読む）
- **観点パネル（エージェントチーム）**: 変更が大きい場合（目安: 複数観点にまたがる/10ファイル超）は、観点ごと（セキュリティ/アーキテクチャ/設計品質/表現・描画系…下記「対象範囲」から該当分）に**並列の code-reviewer サブエージェント**へ分担させ、結果を統合する。統合時は重複指摘をまとめ、観点間で矛盾する指摘は両論併記して人間に委ねる
- 人間が実装文脈のないセッションで直接 `/deep-review` を起動した場合は、そのまま実行してよい
- **自律ループの収束判定は Critical / Warning 件数のみ**で行う。所感・総評は人間向けの出力であり、収束判定に影響させない
...
| 観点 | チェック内容 |
|------|-------------|
| **SSOT整合** | rdd.md / architecture.md との整合性 |
| **Mermaid図** | 構文検証、描画可能性、図と説明の整合 |
| **ADR-lite妥当性** | 決定/理由/代替案/トレードオフの記載 |
| **網羅性** | 変更箇所が設計書に反映されているか |
| **構造品質** | 見出し階層、必須セクション、リンク |
| **用語統一** | プロジェクト内での用語の一貫性 |

### Docコメント転記（コード変更時）

PRにコード変更が含まれる場合、設計書の内容をDocコメントに転記:
...
3. **転記フォーマット**
    ...
    * {関数/クラス名}
    *
    * @description {設計書からの責務・意図}
    * @constraint {設計上の制約があれば}
    * @see {設計書へのパス}
    */
   ```

4. **レビュー指摘として提案**
   - Docコメントが設計書と乖離している → Warning
   - Docコメントが無い → Suggestion（転記内容を提示）
...
#### 3.4 表現・描画系観点（UI/アニメーション変更時）
```
- [ ] 世界観整合: 境界・容器・レイヤーの辻褄が合っているか（見た目の嘘がないか）
- [ ] 未翻訳の要素: ジャンルの定番UI/定番文言がそのまま残っていないか（`project-design-language` のメタファーで言い直せているか）
- [ ] フレーム内の重い処理: 全画面blur・合成等が要素数に比例していないか（層単位でまとめられないか）
- [ ] 決定性・テスト可能性: 状態を持つ必要が本当にあるか
- [ ] SSOT迂回: 色/文言/効果値の直書きがないか
```
