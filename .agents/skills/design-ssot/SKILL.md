...
### 1) Codex側：MCP登録の確認
- `Codex mcp list` を確認し、`figma` が登録されていること
...
- **前提**
  ...
  - Codex（CLI）を実行する環境から、そのMCPのHTTPエンドポイントに到達できること
- **手順（最小）**
  1. `Codex mcp list` を確認し、`figma` が無ければ登録する
     - 例: `Codex mcp add --transport http figma "http://localhost:3845/mcp"`
