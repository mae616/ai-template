...
- ai-template（.Codex/ AGENTS.md doc/）を**自動適用**し、rdd.md に要件定義を反映した状態で開発を開始
...
if [ -f "./AGENTS.md" ] && [ -d "./.Codex/skills/project-init" ]; then
  TEMPLATE_SOURCE="$(pwd)"

# 2. グローバル設定ファイルから読み取り
elif [ -f "$HOME/.Codex/ai-template-path" ]; then
  TEMPLATE_SOURCE="$(cat $HOME/.Codex/ai-template-path)"

# 3. よくあるパスを探索
elif [ -d "$HOME/output/ai-template/.Codex/skills" ]; then
...
### テンプレートパスの永続化（初回のみ）
見つかったパスを `~/.Codex/ai-template-path` に保存して、次回以降自動検出できるようにする。
ユーザーに保存の確認を取ること。
...
| コピー元 | コピー先 | 説明 |
|---------|---------|------|
| `.Codex/skills/` | `.Codex/skills/` | 判断軸・手順系スキル一式 |
| `.Codex/settings.json` | `.Codex/settings.json` | Codex 設定（存在する場合） |
| `AGENTS.md` | `AGENTS.md` | プロジェクトルール |
| `.Codex/rules/` | `.Codex/rules/` | 運用ルール（自動適用） |
...
cp -r "$TEMPLATE_SOURCE/.Codex/skills" "$PROJECT_DIR/.Codex/"

# 設定ファイルをコピー（存在する場合のみ）
[ -f "$TEMPLATE_SOURCE/.Codex/settings.json" ] && cp "$TEMPLATE_SOURCE/.Codex/settings.json" "$PROJECT_DIR/.Codex/"

# AGENTS.md をコピー
cp "$TEMPLATE_SOURCE/AGENTS.md" "$PROJECT_DIR/"

# 運用ルールをコピー
cp -r "$TEMPLATE_SOURCE/.Codex/rules/" "$PROJECT_DIR/.Codex/rules/"
```
...
- [ ] .Codex/skills/ がコピーされ、スキル一覧で認識される
- [ ] AGENTS.md がプロジェクトルートに存在する
...
```bash
# 手動でパスを設定
echo "/path/to/ai-template" > ~/.Codex/ai-template-path
```
