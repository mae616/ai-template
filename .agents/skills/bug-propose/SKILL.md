---
user-invocable: true
description: "調査済みのバグに対して修正策を列挙し、根拠・影響・リスクを比較してIssueコメントに追記する。修正方針の決定、環境・設定系バグの対応策検討で使う。"
---
...
- 出典ポリシーは `AGENTS.md` の原則、および `.Codex/rules/code-quality.md` の「情報源・引用（出典）ポリシー（詳細）」に従い、**根拠を伴う対応策** を整理する
...
- 詳細は `.Codex/rules/code-quality.md` を参照
...
## 検索・評価ポリシー（参照）
詳細は `.Codex/rules/code-quality.md` の「情報源・引用（出典）ポリシー（詳細）」に従う（= `AGENTS.md` の原則を具体化）。
