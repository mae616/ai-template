...
- **`sprint/*` 上で起動された場合**は、対象 Issue 用の `task/*` ブランチを自動作成してから開始する（命名は `.Codex/rules/git.md` 準拠）:
...
- usage 切れは Codex が自動停止に任せる（明示検知不要）
...
- target ブランチは現在 task ブランチが切られた sprint ブランチ（`.Codex/rules/git.md` 参照）
...
**マージ前の停止条件（危険変更チェック）**: 変更が `.Codex/rules/code-quality.md` の危険変更チェックリスト（認証・認可 / 決済 / 個人情報 / DBマイグレーション / 権限設定 / 外部API連携 / 削除処理 / 通知・メール送信）のいずれかに該当する場合、**自律マージせずここで停止**する:
...
- リモートデプロイ
- 任意の従量課金 SaaS への新規呼び出し（Codex API 以外の外部 API）
