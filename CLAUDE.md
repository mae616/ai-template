...
1. **`.claude/session-context.md` を確認** - 前回の続きがあるか
   - ファイルがある場合 → 内容を提示して「続きから再開しますか？」と確認
   - ファイルがない場合 → 次へ

2. **TaskListを確認** - 進行中のタスクがあるか
   - タスクがある場合 → 現在の状態（in_progress/pending）と残タスクを提示してから続行
   - タスクがない場合 → 新規タスクとしてゴール確認へ

実装/レビュー/ドキュメント等のタスクを受けた場合、作業開始前に以下を確認する:
- **ゴール**: 1文で完了状態を記述
- **完了条件**: 具体的なチェックリスト（「〜が完了している」形式）
- **チェックポイント**: 中断対策のコミットタイミング

> 💡 質問や調査のみの場合はスキップしてOK。実装タスクでは必ず確認する。
> 📌 この自動確認は簡易版。詳細な計画が必要な場合は `/session-start` を手動で実行。
